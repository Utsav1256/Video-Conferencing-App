...
function MeetingRoom() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        socket.on('media-state', ({ userId, isMuted, isVideoOff, from, isScreenSharing }) => {
    ...
    return () => {
      ...
      if (socketRef.current) {
        socketRef.current.emit('leave-meeting', { meetingId, userId: user?._id || 'anon' });
  ...
  const handleLeaveMeeting = () => {
    ...
    navigate('/call-ended', { state: { meetingId, duration: durationLabel, participants: participantsCount } });
  };
  ...
  const toggleMute = () => {
    ...
    if (socketRef.current) {
      socketRef.current.emit('media-state', { meetingId, userId: user?._id || 'anon', isMuted: next, isVideoOff, from: socketRef.current.id });
    }
  };
  ...
  const toggleVideo = () => {
    ...
    if (socketRef.current) {
      socketRef.current.emit('media-state', { meetingId, userId: user?._id || 'anon', isMuted, isVideoOff: next, from: socketRef.current.id });
    }
  };
  ...
  const startScreenShare = async () => {
    try {
      ...
      if (socketRef.current) {
        socketRef.current.emit('media-state', { meetingId, userId: user?._id || 'anon', isMuted, isVideoOff, isScreenSharing: true, from: socketRef.current.id });
      }
      screenTrack.onended = () => {
        stopScreenShare();
      };
  ...
  const stopScreenShare = async () => {
    ...
    if (socketRef.current) {
      socketRef.current.emit('media-state', { meetingId, userId: user?._id || 'anon', isMuted, isVideoOff, isScreenSharing: false, from: socketRef.current.id });
    }
  };
  ...
  return (
    <div className="meeting-room">
      ...
      <div className="meeting-content">
        {isScreenSharing && (
          <div style={{ position: 'absolute', top: 64, left: '50%', transform: 'translateX(-50%)', background: '#111827', color: '#fff', padding: '6px 10px', borderRadius: 8, zIndex: 5 }}>
        ...
        <div className="video-grid-container">
          ...
          {focusedSocketId ? (
            <div style={{ marginBottom: 16 }}>
              <div className="video-tile" style={{ width: '100%', aspectRatio: '16 / 9' }}>
                {focusedSocketId === 'self' ? (
                  isVideoOff ? (
                    <div className="video-placeholder">
                      <div className="avatar"><span className="avatar-text">{getInitials(user?.name || 'You')}</span></div>
                    </div>
                  ) : (
                    <video ref={localVideoRef} autoPlay playsInline muted className="video-element" />
                  )
                ) : (
                  ((peersRef.current.get(focusedSocketId) && peersRef.current.get(focusedSocketId).isVideoOff)) ? (
                    <div className="video-placeholder">
                      <div className="avatar"><span className="avatar-text">{getInitials(getPeerName(focusedSocketId))}</span></div>
                    </div>
                  ) : (
                    <video
                      ref={(el) => {
                ...
                <div className="video-info">
                  <span className="participant-name">{focusedSocketId === 'self' ? (user?.name || 'You') : getPeerName(focusedSocketId)}</span>
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className={`video-grid ${[...remotePeers, { socketId: 'self' }].length <= 2 ? 'grid-1-2' : remotePeers.length <= 3 ? 'grid-2-2' : 'grid-3-3'}`}>
            <div
            ...
            {remotePeers.map(({ socketId }) => (
              <div
                ...
                onClick={() => {
                  ...
                  const entry = peersRef.current.get(socketId);
                  if (entry && entry.videoEl && !entry.isVideoOff) enterFullscreen(entry.videoEl);
                }}
                style={{ cursor: 'pointer' }}
              >
                {((peersRef.current.get(socketId) && peersRef.current.get(socketId).isVideoOff)) ? (
                  <div className="video-placeholder">
                    <div className="avatar"><span className="avatar-text">{getInitials((peersRef.current.get(socketId) && peersRef.current.get(socketId).name) || 'Peer')}</span></div>
                  </div>
                ) : (
                  <video
                    ref={(el) => {
                ...
                <div className="video-info">
                  <span className="participant-name">{(peersRef.current.get(socketId) && peersRef.current.get(socketId).name) || 'Peer'}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="meeting-controls">
        {toasts.length > 0 && (
          <div style={{ position: 'fixed', bottom: 16, right: 16, zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
            {toasts.map(t => (
              <div key={t.id} style={{ background: '#111827', color: '#fff', padding: '10px 14px', borderRadius: 10, boxShadow: '0 10px 24px rgba(0,0,0,0.25)', maxWidth: 360 }}>
                {t.msg}
              </div>
            ))}
          </div>
